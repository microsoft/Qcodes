...
class Station(Metadatable, DelegateAttributes):
    ...
    def __init__(self, *components, monitor=None, default=True,
        ...
        """
        *components: components to add immediately to the Station.
            can be added later via self.add_component

        monitor: the object that monitors the system continuously

        default (default True): is this station the default, which gets
            used in Loops and elsewhere that a Station can be specified

        update_snapshot (default True): immediately update the snapshot
