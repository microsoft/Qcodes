...
# Adapted by QDevil from "qdev\QDac_channels.py" in the instrument drivers package - kudos to QDEV and the Microsoft quantum computing consortium for making that
# AK QDevil 2019-06-24
...
class QDacChannel(InstrumentChannel):
    ...
    Exposes chan.v, chan.vrange, chan.vmin, chan.vmax, chan.slope, chan.i, chan.irange, chan.slope
    Always set v to zero before changing vrange or irange
    """
    ...
    def __init__(self, parent, name, channum):
        ...
        self.add_parameter('v',
                           ...
                           get_parser=float,
                           vals=vals.Numbers(-9.99, 9.99)  # Initial values. Are updated at initialisation and when vrange is changed
                           )

        self.add_parameter('vrange',
                           label='Channel {} range.'.format(channum),
                           set_cmd=partial(self._parent._set_vrange, channum),
                           get_cmd='vol {}'.format(channum),
                           get_parser=int,
                           vals=vals.Enum(0, 1)
                           )

        self.add_parameter('vmin',
                           label='Channel {} min voltage'.format(channum),
                           unit='V',
                           set_cmd=None,
                           get_cmd=partial(self._parent._get_vmin, channum),
                           get_parser=float,
                           )

        self.add_parameter('vmax',
                           label='Channel {} max voltage'.format(channum),
                           unit='V',
                           set_cmd=None,
                           get_cmd=partial(self._parent._get_vmax, channum),
                           get_parser=float,
                           )
        ...
        self.add_parameter('irange',
                           ...
                           set_cmd=partial(self._parent._set_irange, channum),
...
class QDac(VisaInstrument):
    ...
    Based on "PM10091-7 QDAC SW1.07.pdf"
    Tested with Firmware Version: 1.07

    The driver assumes that the instrument is ALWAYS in verbose mode OFF
    and sets this as part of the initialization.
    """

    # Range factors for QDevil QDAC 1.0
    voltage_range_status = {'X 1': 0, 'X 0.1': 1}
    ...
    def __init__(self,
        ...
        Args:
            ...
            num_chans (int): Number of channels to assign. Default: 48 (Legacy, not required)
        ...
        # isQDevilQDAC  = self._check_if_qdevilqdac()
    ...
    def _set_voltage(self, chan, v_set):
        ...
        outofrange = False
        if v_set > self.vranges[chan][atten]['Max']:
            v_set = self.vranges[chan][atten]['Max']
            outofrange = True
        ...
        if outofrange:
            log.warning('Requested voltage outside reachable range.' +
                        ...
                        'Note: _get_latest not up to date')
        ...
        if xor(old, switchint):
            ...
            if (switchint == 0): 
                ...
                if oldvoltage > self.vranges[chan][switchint]['Max']:
                    newvoltage = self.vranges[chan][switchint]['Max']
                    log.warning('Voltage is outside the bounds of the low range and is therefore clipped.')
                elif oldvoltage < self.vranges[chan][switchint]['Min']:
                    newvoltage = self.vranges[chan][switchint]['Min']
                    log.warning('Voltage is outside the bounds of the low range and is therefore clipped.')
                else:
                    newvoltage = oldvoltage
    ...
    def _set_irange(self, chan, switchint):
        ...
        The output voltage should be set to zero before changing the range,
        otherwise a spike will occur on the output. When switching from high
        current range to low current range the volage (if not zero) may drop if
        the current draw is exceeding the max current in the low current range.
        """
        ...
        if xor(old, switchint):
            if (old_vrange == 1) and (switchint == 1):
                log.warning('''
                            QDAC can not be in high current range and low voltage range and at the same time. 
                            Current range has NOT been changed and _get_latest is NOT up to date
                            ''')
                self.channels[chan-1].irange._save_val(0)
            else:
                self.write('cur {} {}'.format(chan, switchint))
    ...
    def _get_status(self, readcurrents=False):
        ...
        The `status` call generates 27 or 51 lines of output. Send the command and
        ...
        8\t  0.000000\t\tX 1\t\tpA\n
        7\t  0.000000\t\tX 1\t\tpA\n
        ... (all 24/48 channels like this in a somewhat peculiar order)
    ...
    def _setsync(self, chan, sync):
        ...
        Args:
            ...
            sync (int): The associated sync output (1-3 on 24 ch units or 1-5 on 48 ch units). 0 means 'unassign'
        """
    ...
    def _get_firmware_version(self):
        """
        Check if the "version" command reponds. If so we probbaly have a QDevil QDAC,
        and the version number is returned. Otherwise 0.0 is returned.
        """
        self.write('version')
        FW_str = self._write_response
        if (not ("Unrecognized command" in FW_str)) and ("Software Version: " in FW_str):
