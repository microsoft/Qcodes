...
class TUpgraderFunction(Protocol):
    def __call__(
        self, conn: AtomicConnection, show_progress_bar: bool = True
    ) -> None: ...
...
def _get_no_of_runs(conn: AtomicConnection) -> int:
...
def upgrader(func: TUpgraderFunction) -> TUpgraderFunction:
    ...
    @wraps(func)
    def do_upgrade(conn: AtomicConnection, show_progress_bar: bool = True) -> None:
...
def perform_db_upgrade(conn: AtomicConnection, version: int = -1) -> None:
    """
...
def perform_db_upgrade_0_to_1(
    conn: AtomicConnection, show_progress_bar: bool = True
) -> None:
    """
