...
# A convenient class to keep track of vectors representing physical
# fields. The idea is that a vector instance stores a representation in
# cartesian, spherical and cylindrical coordinates. Giving either (x, y,
# z) values or (rho, phi, z) values or (r, theta, phi) values at
# instantiation we will calculate the other representation immediately.
...
class FieldVector(object):
    ...
    def __init__(self, x=None, y=None, z=None,
                 r=None, theta=None, phi=None, rho=None):
        """
        Parameters:
            x (float, optional): represents the norm of the projection
                of the vector along the x-axis
            y (float, optional): represents the norm of the projection
                of the vector along the y-axis
            z (float, optional): represents the norm of the projection
                of the vector along the z-axis
            r (float, optional): represents the norm of the vector
            theta (float, optional): represents the angle of the
                vector with respect to the positive z-axis
            rho (float, optional): represents the norm of the
                projection of the vector on to the xy-plane
            phi (float, optional): represents the angle of rho with
                respect to the positive x-axis

        Note: All inputs are optional, however the user needs to
        either give (x, y, z) values, (r, theta, phi) values or (phi,
        rho, z) values for meaningful computation
        """
        self._x = x
        self._y = y
        self._z = z
    ...
    def _set_attribute_value(self, attr_name, value):
        ...
        if attr_value is None:
            ...
            if not np.isclose(attr_value, value):
                raise ValueError("Computed value of {} inconsistent "
                                 "with given value".format(attr_name))
    ...
    def _compute_unknowns(self):
        """
        Compute all coordinates. To do this we need either the set (x,
        y, z) to contain no None values, or the set (r, theta, phi),
        or the set (rho, phi, z). Given any of these sets, we can
        recompute the rest.

        This function will raise an error if there are contradictory
        inputs (e.g. x=3, y=4, z=0 and rho=6).
        """
        ...
        for f in [
            ...
            new_values = f()
            # this will return None if any of the function arguments is None.
    ...
    def set_vector(self, **new_values):
        ...
        Examples:
            ...
            # the next line should raise a value error:
            # "Can only set vector with a complete value
            >>> f.set_vector(x=9, y=0)
            # set"
            >>> f.set_vector(x=9, y=0, r=3)
            # although mathematically it is possible to compute the complete
            # vector from the values given, this is too hard to implement
            # with generality (and not worth it) so this to will raise the
            # above mentioned ValueError
        """
    ...
    def set_component(self, **new_values):
        ...
        disallowed for the user to set vector components manually as
        this can lead to inconsistencies (e.g. x and rho are not
        independent of each other, setting one has to effect the other)

        Examples:
            >>> f = FieldVector(x=2, y=3, z=4)
            >>> f.set_component(r=10)
            # Since r is part of the set (r, theta, phi) representing
            # spherical coordinates, setting r means that theta and
            # phi are kept constant and only r is changed. After
            # changing r, (x, y, z) values are recomputed, as is the
            # rho coordinate. Internally we arrange this by setting x,
            # y, z and rho to
            # None and calling self._compute_unknowns()

        Parameters:
            new_values (dict): keys representing parameter names
                and values the values to be set
        """

        if len(new_values) > 1:
            raise NotImplementedError("Cannot set multiple components at once")
            # It is not easy to properly implement a way of setting
            # multiple components at the same time as we need to check
            # for inconsistencies. E.g. how do we handle the following
            # situation:
        ...
        # Setting x (for example), we will keep y and z untouched, but
        # set r, theta, phi and rho to None
