...
class DacChannel(InstrumentChannel, DacReader):
    ...
    def _set_fine_voltage(self, voltage):
        ...
        if self._channel == 0:
            ...
            raise RuntimeError("Fine mode only works for Chan 0 and 1")
        coarse_round = round(voltage,2)
        coarse_part = self._dac_code_to_v(self._dac_v_to_code(coarse_round-0.01))
...
class Decadac(VisaInstrument, DacReader):
    ...
    def __init__(self, name, address, min_val=-5, max_val=5, **kwargs):
        ...
        # Create channels
        channels = ChannelList(self, "Channels", DacChannel, snapshotable=True)
