...
def _make_forged_sequence():
    ...
    seq[N]["content"] = {
        1: {"data": random_element(num_chans), "sequencing": {"nreps": 2}},
        2: {"data": random_element(num_chans), "sequencing": {"nreps": 2}},
    }
...
def test_SML_successful_generation_vary_length(N) -> None:
    ...
    ejt = [0] * N
    goto = [0] * N
    wfm_names = [[f"pos{pos}ch{ch}" for ch in range(1, 3)] for pos in range(N)]
