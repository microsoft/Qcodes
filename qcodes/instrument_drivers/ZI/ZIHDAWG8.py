...
class ZIHDAWG8(Instrument):
    ...
    @staticmethod
    def generate_csv_sequence_program(wave_names: list,
                                      channels: Optional[list] = None) -> str:
        """
        Generates and returns a sequencing program that plays the given waves on
         the given channels. There has to be a CSV file with a corresponding
         name to a wave in wave_names.

        Args:
            wave_names: List of wave names that are to be played.
            channels: Channels to play the waveforms on.

        Returns:
            A sequencing program that can be uploaded to the device.
        """
        ...
        if channels is None:
            ...
            argument_string = ('{}, "{}"' * len(wave_names)).replace('}"{',
                                                                     '}", {')
            waves = argument_string.format(
                *[value for pair in zip(channels, wave_names) for value in
                  pair])
