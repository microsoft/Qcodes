...
def test_basic_subscription(dataset, basic_subscriber):
    ...
    assert len(dataset.dsi.writer.subscribers) == 1
    assert list(dataset.dsi.writer.subscribers.keys()) == [sub_id]
    ...
    for x in range(10):
        ...
        def assert_expected_state():
            assert dataset.dsi.writer.subscribers[sub_id].state == expected_state

        assert_expected_state()

    dataset.unsubscribe(sub_id)

    assert len(dataset.dsi.writer.subscribers) == 0
    assert list(dataset.dsi.writer.subscribers.keys()) == []
    ...
    triggers = atomic_transaction(
        dataset.dsi.writer.conn, get_triggers_sql).fetchall()
...
def test_subscription_from_config(dataset, basic_subscriber):
    ...
    with default_config(user_config=config):
        ...
        assert len(dataset.dsi.writer.subscribers) == 2
        assert list(dataset.dsi.writer.subscribers.keys()) == [sub_id, sub_id_c]
        ...
        for x in range(2):
            ...
            def assert_expected_state():
                assert dataset.dsi.writer.subscribers[sub_id].state == expected_state
                assert dataset.dsi.writer.subscribers[sub_id_c].state == expected_state
