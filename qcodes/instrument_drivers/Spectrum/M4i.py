...
# TODO: whenever an error occurs (including validation errors) the python console needs to be restarted
...
class M4i(Instrument):
    ...
    def __init__(self, name, cardid='spcm0', **kwargs):
        ...
        self.add_parameter('available_card_modes',
                           ...
                           get_cmd=partial(self._param32bit, pyspcm.SPC_AVAILCARDMODES),
        ...
        self.add_parameter('available_channel_and_mask',
                           ...
                           get_cmd=partial(self._param32bit, pyspcm.SPC_TRIG_CH_AVAILANDMASK0),
        ...
        self.add_parameter('external_trigger_min_level',
                           ...
                           units='mV',
                           get_cmd=partial(self._param32bit, pyspcm.SPC_TRIG_EXT_AVAIL0_MIN),
                           docstring='returns the minimum trigger level')
        self.add_parameter('external_trigger_max_level',
                           label='external trigger max level',
                           units='mV',
                           get_cmd=partial(self._param32bit, pyspcm.SPC_TRIG_EXT_AVAIL0_MAX),
        ...
        self.add_parameter('bytes_per_sample',
                           ...
                           get_cmd=partial(self._param32bit, pyspcm.SPC_MIINST_BYTESPERSAMPLE),
        ...
        self.add_parameter('enable_channels',
                           ...
                           get_cmd=partial(self._param32bit, pyspcm.SPC_CHENABLE),
                           set_cmd=partial(self._set_param32bit, pyspcm.SPC_CHENABLE),
                           vals=Enum(1, 2, 4, 8, 3, 5, 9, 6, 10, 12, 15),
                           docstring='Set and get enabled channels')

        # analog input path functions
        # TODO: change Enum validator to set_parser for the numbered functions if we want string inputs
        ...
        for i in [0, 1, 2, 3]:
            self.add_parameter('input_path_{}'.format(i),
                               ...
                               get_cmd=partial(self._param32bit, getattr(pyspcm, 'SPC_PATH{}'.format(i))),
                               set_cmd=partial(self._set_param32bit, getattr(pyspcm, 'SPC_PATH{}'.format(i))),
                               vals=Enum(0, 1),
                               docstring='Set and get analog input path for channel {}'.format(i))

            # channel range functions
            # TODO: check the input path to set the right validator (either by directly calling input_path_x() or by storing a variable)
            self.add_parameter('range_channel_{}'.format(i),
                               label='range channel {}'.format(i),
                               get_cmd=partial(self._param32bit, getattr(pyspcm, 'SPC_AMP{}'.format(i))),
                               set_cmd=partial(self._set_param32bit, getattr(pyspcm, 'SPC_AMP{}'.format(i))),
            ...
            self.add_parameter('termination_{}'.format(i),
                               ...
                               get_cmd=partial(self._param32bit, getattr(pyspcm, 'SPC_50OHM{}'.format(i))),
                               set_cmd=partial(self._set_param32bit, getattr(pyspcm, 'SPC_50OHM{}'.format(i))),
                               vals=Enum(0, 1),
            ...
            self.add_parameter('ACDC_coupling_{}'.format(i),
                               ...
                               get_cmd=partial(self._param32bit, getattr(pyspcm, 'SPC_ACDC{}'.format(i))),
                               set_cmd=partial(self._set_param32bit, getattr(pyspcm, 'SPC_ACDC{}'.format(i))),
                               vals=Enum(0, 1),
            ...
            self.add_parameter('anti_aliasing_filter_{}'.format(i),
                               ...
                               get_cmd=partial(self._param32bit, getattr(pyspcm, 'SPC_FILTER{}'.format(i))),
                               set_cmd=partial(self._set_param32bit, getattr(pyspcm, 'SPC_FILTER{}'.format(i))),
                               vals=Enum(0, 1),
                               docstring='if 1 selects bandwidth limit, if 0 sets to full bandwidth for channel {}'.format(i))

        # acquisition modes
        # TODO: If required, the other acquisition modes can be added to the validator
        ...
        self.add_parameter('timeout',
                           label='timeout',
                           get_cmd=partial(self._param32bit, pyspcm.SPC_TIMEOUT),
                           units='ms',
                           set_cmd=partial(self._set_param32bit, pyspcm.SPC_TIMEOUT),
        ...
        # TODO: improve the validators to make them take into account the current state of the instrument
        ...
        self.add_parameter('segment_size',
                           label='segment size',
                           get_cmd=partial(self._param32bit, pyspcm.SPC_SEGMENTSIZE),
                           set_cmd=partial(self._set_param32bit, pyspcm.SPC_SEGMENTSIZE),
        ...
        self.add_parameter('clock_mode',
                           ...
                           vals=Enum(pyspcm.SPC_CM_INTPLL, pyspcm.SPC_CM_QUARTZ2, pyspcm.SPC_CM_EXTREFCLOCK, pyspcm.SPC_CM_PXIREFCLOCK),
        ...
        self.add_parameter('trigger_or_mask',
                           ...
                           vals=Enum(pyspcm.SPC_TMASK_NONE, pyspcm.SPC_TMASK_SOFTWARE, pyspcm.SPC_TMASK_EXT0, pyspcm.SPC_TMASK_EXT1),
        ...
        self.add_parameter('trigger_and_mask',
                           ...
                           vals=Enum(pyspcm.SPC_TMASK_NONE, pyspcm.SPC_TMASK_EXT0, pyspcm.SPC_TMASK_EXT1),
        ...
        for l in [0, 1]:
            self.add_parameter('external_trigger_level_{}'.format(l),
                               ...
                               get_cmd=partial(self._param32bit, getattr(pyspcm, 'SPC_TRIG_EXT0_LEVEL{}'.format(l))),
                               set_cmd=partial(self._set_param32bit, getattr(pyspcm, 'SPC_TRIG_EXT0_LEVEL{}'.format(l))),
        ...
        for i in [0, 1, 2, 3]:
            self.add_parameter('trigger_mode_channel_{}'.format(i),
                               ...
                               get_cmd=partial(self._param32bit, getattr(pyspcm, 'SPC_TRIG_CH{}_MODE'.format(i))),
                               set_cmd=partial(self._set_param32bit, getattr(pyspcm, 'SPC_TRIG_CH{}_MODE'.format(i))),
            ...
            for l in [0, 1]:
                self.add_parameter('trigger_channel_{}_level_{}'.format(i, l),
                                   ...
                                   get_cmd=partial(self._param32bit, getattr(pyspcm, 'SPC_TRIG_CH{}_LEVEL{}'.format(i, l))),
                                   set_cmd=partial(self._set_param32bit, getattr(pyspcm, 'SPC_TRIG_CH{}_LEVEL{}'.format(i, l))),
        ...
        self.add_parameter('card_available_length',
                           ...
                           set_cmd=partial(self._set_param32bit, pyspcm.SPC_DATA_AVAIL_CARD_LEN),
    ...
    def set_channel_settings(self, i, mV_range, input_path, termination, coupling, compensation):
        ...
        getattr(self, 'range_channel_{}'.format(i))(mV_range)  # note: set after voltage range
        getattr(self, 'ACDC_offs_compensation_{}'.format(i))(compensation)  # can only be used with DC coupling and 50 Ohm path (hf)
    ...
    def set_channel_OR_trigger_settings(self, i, trig_mode, bitlevel0, bitlevel1=None):
        ...
        getattr(self, 'trigger_mode_channel_{}'.format(i))(trig_mode)  # trigger mode
    ...
    # TODO: the data also needs to be organized nicely (currently it interleaves the data)
    def multiple_trigger_acquisition(self, mV_range, memsize, seg_size, posttrigger_size):
        ...
        self.general_command(pyspcm.M2CMD_CARD_START | pyspcm.M2CMD_CARD_ENABLETRIGGER | pyspcm.M2CMD_CARD_WAITREADY)
        ...
        # data acquisition
        self._def_transfer64bit(pyspcm.SPCM_BUF_DATA, pyspcm.SPCM_DIR_CARDTOPC, 0, data_pointer, 0, 2 * memsize)
    ...
    def get_error_info32bit(self):
        ...
        pyspcm.spcm_dwGetErrorInfo_i32(self.hCard, pyspcm.byref(dwErrorReg), pyspcm.byref(lErrorValue), None)
    ...
    def get_card_memory(self, verbose=0):
        data = pyspcm.int64(0)
        pyspcm.spcm_dwGetParam_i64(self.hCard, pyspcm.SPC_PCIMEMSIZE, pyspcm.byref(data))
        if verbose:
