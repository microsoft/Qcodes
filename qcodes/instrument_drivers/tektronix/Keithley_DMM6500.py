...
class Keithley_DMM6500(VisaInstrument):
    """
    Driver for the Keithley DMM6500 multimeter.
    """
...
#                           get_cmd=self._read_next_value)
        ...
        self.add_parameter('buffer_readings',
...
#        self.write('FORM:ELEM READ')
    ...
    def _get_mode_param(self, parameter, parser):
        ...
        if mode != 'none':
            ...
            raise Exception('Unavailable in digitizer mode.')
    ...
    def trace_data(self, start_index, end_index):
        """Read out the data in a trace.

        Args:
            start_index (int): 1 or larger
            end_index (int):
        """
        response = self.ask(':TRACe:DATA? {0:d}, {1:d}'.format(
            start_index, end_index))
        if self.data_format() == 'ASC':
            data = array(response.split(',')).astype(float64)
        else:
            raise ValueError('Other data types not implemented yet.')

        return data

    def read(self):
        """Make measurements and return the last reading from the buffer.

        Notes:
            See manual 13-12
            Number of readings depends on the :SENSE:COUNTS setting
        """
