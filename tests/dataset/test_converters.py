...
def test_convert_v0_to_newer(some_paramspecs) -> None:
    ...
    interdeps = InterDependencies(
        pgroup1["ps1"], pgroup1["ps2"], pgroup1["ps3"], pgroup1["ps4"], pgroup1["ps6"]
    )
...
def _assert_dicts_are_related_as_expected(v0, v1, v2):
    assert (
        v1["interdependencies"]
        == old_to_new(InterDependencies._from_dict(v0["interdependencies"]))._to_dict()
    )
    assert v1["version"] == 1
    assert len(v1) == 2
...
def test_construct_current_rundescriber_from_fake_v4(some_interdeps) -> None:
    ...
    v4["foobar"] = {"foo": ["bar"]}  # type: ignore[typeddict-unknown-key]  # ty: ignore[invalid-key]
    rds1 = RunDescriber._from_dict(v4)
    rds_upgraded = from_dict_to_current(v4)
    v3 = v4.copy()
    v3.pop("foobar")  # type: ignore[typeddict-item]  # ty: ignore[invalid-key]
    v3["version"] = 3
    assert rds1._to_dict() == v3
    assert rds_upgraded._to_dict() == v3
