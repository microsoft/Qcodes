...
def test_literal_validator() -> None:
    ...
    with pytest.raises(ValueError, match="5 is not a member of "):
        a123_val.validate(5, context="Outside range")  # pyright: ignore[reportArgumentType]  # ty: ignore[invalid-argument-type]

    with pytest.raises(ValueError, match="some_str is not a member of "):
        a123_val.validate("some_str", context="Wrong type")  # pyright: ignore[reportArgumentType]  # ty: ignore[invalid-argument-type]
