from typing import Union, Dict, Sequence, List, Optional, Tuple
...
import qcodes as qc
from qcodes.dataset.data_set import get_guids_from_run_spec, connect, load_by_guid
...
def guids_from_list_str(s: str) -> Optional[Tuple[str, ...]]:
    ...
    captured_guid = f"(?:\\s*['\"]?({guid})['\"]?\\s*)"
    open_parens = r"[\[\(\{]"
    close_parens = r"[\]\)\}]"
    m = re.match(
        f'^\s*{open_parens}?'
        f'(?:{captured_guid},)*{captured_guid}?'
        f'{close_parens}?\s*$'
        , s)
    if m is None:
        return None
