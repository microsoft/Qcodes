...
def test_not_implemented_error():
    ...
    with pytest.raises(
        NotImplementedError,
        match="has un-implemented Abstract Parameter"
    ):
...
def test_unit_value_error():
    ...
    with pytest.raises(
        ValueError,
        match="This is inconsistent with the unit defined"
    ):
...
def test_exception_in_init():
    ...
    assert name in [
        ins.name for ins in VoltageSourceInitException.instances()
    ]
    instance.close()
