...
def test_complex_types() -> None:
    e = NumpyJSONEncoder()
    assert e.encode(complex(1, 2)) == '{"__dtype__": "complex", "re": 1.0, "im": 2.0}'
    for complex_type in numpy_complex:
        assert (
            e.encode(complex_type(complex(1, 2)))
...
def test_UFloat_type() -> None:
    e = NumpyJSONEncoder()
    assert (
        e.encode(uncertainties.ufloat(1.0, 2.0))
...
def test_numpy_array() -> None:
    ...
    assert e.encode(np.array([1, 0, 0])) == "[1, 0, 0]"

    assert e.encode(np.arange(1.0, 3.0, 1.0)) == "[1.0, 2.0]"

    assert (
        e.encode(np.meshgrid((1, 2), (3, 4))) == "[[[1, 2], [1, 2]], [[3, 3], [4, 4]]]"
    )
...
def test_non_serializable() -> None:
    ...
    assert e.encode(Dummy()) == '"i am a dummy with \\\\ slashes /"'
...
def test_object_with_serialization_method() -> None:
    ...
    assert e.encode(Dummy()) == '{"i_am_actually": "a_dict_addict"}'
