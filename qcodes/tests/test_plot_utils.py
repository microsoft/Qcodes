...
# on
from qcodes.tests.dataset.test_dataset_basic import dataset, experiment, empty_temp_db
...
# this follows the 'factories as fixture' pattern
# for more info see here:
# https://docs.pytest.org/en/latest/fixture.html#factories-as-fixtures
# @fixture()
# def dataset_with_outliers_generator(dataset):
#     def make_dataset(data_offset=5, low_outlier=-3,
#                  high_outlier=1, background_noise=True):
#         x = ParamSpec('x', 'numeric', label='Flux', unit='e^2/hbar')
#         t = ParamSpec('t', 'numeric', label='Time', unit='s')
#         z = ParamSpec('z', 'numeric', label='Majorana number', unit='Anyon',
...
#         dataset.add_parameter(t)
#         dataset.add_parameter(z)
...
#             if counter == round(npoints/2.1):
#                 data[round(npoints/2.5)] = low_outlier
#             dataset.add_results([{'x': xv, 't': tv, 'z': z}
#                                 for z, tv in zip(data, tvals)])
...
# it would be desirable to give these fixtures the scope of the module,
# however, the fixture `dataset` on which they depend is only defined for
# the scope of a function
# @fixture(scope='function')
# def dataset_with_data_outside_iqr_high_outlier(dataset_with_outliers_generator):
#     return dataset_with_outliers_generator(data_offset=2, low_outlier=-2,
#                                           high_outlier=3 )

# @fixture(scope='function')
# def dataset_with_data_outside_iqr_low_outlier(dataset_with_outliers_generator):
#     return dataset_with_outliers_generator(data_offset=2, low_outlier=-2,
#                                           high_outlier=3 )

# @fixture(scope='function')
# def dataset_with_outliers(dataset_with_outliers_generator):
#     return dataset_with_outliers_generator(low_outlier=-3,
#                                            high_outlier=3,
...
def test_extend(dataset_with_outliers):
    ...
    ax, cb = plot_by_id(run_id, auto_color_scale=False)
    assert cb[0].extend == 'neither'
    ax, cb = plot_by_id(run_id, auto_color_scale=True, cutoff_percentile=(0, 0.5))
    assert cb[0].extend == 'min'
    ax, cb = plot_by_id(run_id, auto_color_scale=True, cutoff_percentile=(0.5, 0))
