...
class WavelengthAxis(qc.Parameter):
    ...
    def get_raw(self) -> np.ndarray:
        return self.root_instrument.calib(np.arange(0, 3648))
...
class IntensitySpectrum(qc.ParameterWithSetpoints):
    ...
    def get_raw(self) -> np.ndarray:
        ...
        avg = self.root_instrument.averaging()
        frames = []
        
        for i in range(avg):
            frame = self.root_instrument.get_single_frame()
            frames.append(frame - self.root_instrument.drkdata)
...
class MightexSSE_1304_U(qc.Instrument):
    ...
    def init_device(self):
        ''' Prepares the spectrometer for recording. '''
